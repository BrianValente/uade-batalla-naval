...
def test_new_match():
    ...
    match.add_ship(1, {"x": 0, "y": 0, "size": 3, "orientation": "horizontal"})
    match.add_ship(2, {"x": 0, "y": 0, "size": 3, "orientation": "vertical"})
    assert match.get_board(1)[0][0] == MatchCellState.SHIP

    match.add_shot(1, {"x": 0, "y": 0})
    match.add_shot(2, {"x": 0, "y": 0})
    assert match.get_board(1)[0][0] == MatchCellState.HIT


def test_loaded_match():
    match_1 = Match("Player 1", "Player 2")
    match_1.add_ship(1, {"x": 0, "y": 0, "size": 3, "orientation": "horizontal"})
    match_1.add_shot(1, {"x": 0, "y": 0})
...
def test_out_of_bounds_ship():
    match = Match("Player 1", "Player 2")
    try:
        match.add_ship(1, {"x": 8, "y": 0, "size": 3, "orientation": "horizontal"})
        assert False
    except ValueError:
        assert True
